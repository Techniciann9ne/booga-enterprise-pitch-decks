...
- **Booga Agents Pricing Deck** (`artifacts/booga-agents-pricing-deck`, slug `booga-agents-pricing-deck`): the **confidential Plans & Pricing leave-behind** (Document 3 of 3) — an 8-slide designed document sent to a prospect on request after a meeting. Slides: Cover, Value Frame (price-vs-hours model with bracketed inputs), Three-Step Ladder (data→compute→governance + at-a-glance comparison table), What Each Plan Includes (Team/Business/Enterprise cards), What a Rollout Looks Like ([legal example] sizing, recommends Enterprise), One Platform (capability band), Pricing and Terms, Next Step (Booga One scale-up line + contact). **Every page is marked "Confidential."** All pricing numbers are lifted verbatim from `attached_assets/plans-and-pricing-brief_*.md` (source of truth); structure follows `attached_assets/pricing-leave-behind-brief_*.md`. Reuses the exact brand system (`src/index.css` tokens, Inter, `BrandMark`) and the same `[bracket]` placeholder convention. **Pricing must never appear on the live or teaser decks — this confidential leave-behind only.** After editing: `pnpm --filter @workspace/booga-agents-pricing-deck run validate-slides`, then restart `artifacts/booga-agents-pricing-deck: web`.
